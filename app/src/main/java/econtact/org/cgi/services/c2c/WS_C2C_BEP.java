...
//	}
	...
	public String doVirtualHold_CIC(String url, Cliente cliente){
		InputStream inputStream = null;
		String result = "";
		try {
			Log.d("doVirtualHold_CIC", "Inicio");
			Log.d("doVirtualHold_CIC", "URL="+url);
			// 1. create HttpClient
			...
			jsonObject.accumulate("target", "Grupo_Chat");
			jsonObject.accumulate("targettype", "Workgroup");
			jsonObject.accumulate("subject", "Callback desde Android Mobile");

			JSONObject datosCliente = new JSONObject();
			datosCliente.accumulate("name", cliente.getNombre() + " " + cliente.getApellido());
			datosCliente.accumulate("credentials", "");
			datosCliente.accumulate("telephone", cliente.getTelefono());

			jsonObject.accumulate("participant", datosCliente);
			jsonObject.accumulate("language", "es");
			jsonObject.accumulate("clientToken", "deprecated");


			// 4. convert JSONObject to JSON to String
			json = jsonObject.toString();

			Log.d("doVirtualHold_CIC", "JSON="+json);
			// ** Alternative way to convert cliente object to JSON string usin Jackson Lib
	...
	public String doChat_CIC(String url, Cliente cliente){
		InputStream inputStream = null;
		String result = "";
		try {
			Log.d("doChat_CIC", "Inicio");
			Log.d("doChat_CIC", "URL="+url);
			// 1. create HttpClient
			HttpClient httpclient = new DefaultHttpClient();
			...
			// 3. build jsonObject
			JSONObject jsonObject = new JSONObject();
			jsonObject.accumulate("supportedContentTypes", "text/plain");
			jsonObject.accumulate("target", "Grupo_Chat");
			jsonObject.accumulate("targettype", "Workgroup");
			jsonObject.accumulate("subject", "Chat desde Android Mobile");
			...
			jsonObject.accumulate("transcriptRequired", false);
			jsonObject.accumulate("emailAddress", cliente.getCorreo());
			jsonObject.accumulate("language", "es");
			jsonObject.accumulate("clientToken", "deprecated");
